...
class SparseTracker(object):
    ...
    # 깊이 계산후 깊이에 따라 그룹을 나눠서 mask index를 반환
    ...
    def DCM(self, detections, tracks, activated_starcks, refind_stracks, levels, thresh, is_fuse):
        ...
        if len(track_mask) != 0:
            ...
            # detection과 track의 깊이 레이어 수가 다른 경우
            ...
            for dm, tm in zip(det_mask, track_mask):
                ...
                # 이번 레이어에서 매칭되지 않은 물체는 다음 레이어에서도 비교
    ...
    def update(self, output_results, curr_img = None):
        ...
        removed_stracks = []
        # current detections
        bboxes = output_results[:,:4].cpu().numpy()# x1y1x2y2 
        ...
        # tracks preprocess
        unconfirmed = []    #unconfirmed : 이전 프레임에서 발견되지 않은 물체
        tracked_stracks = []  #tracked_stracks : 추적되고 있던 물체, type: list[STrack]
